...
public class CE_Unprotect implements CommandExecutor {
		...
		if (sender instanceof Player) {
			...
			if (target != null) {
				...
				if (args.length == 1) {
					...
					plugin.playerSettings.blacklistRemove(player, Material
	...
	private void sendSuccessMessage(Player player, String blockName,
			String protectName) {
		plugin.say(player, ChatColor.GREEN, Messages.getString("CE_Unprotect.9") + blockName.toLowerCase() //$NON-NLS-1$
