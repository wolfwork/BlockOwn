...
public class CE_Whitelist implements CommandExecutor {

	BlockOwn plugin;
	public CE_Whitelist(BlockOwn plugin){
		this.plugin=plugin;
	}
		@Override
				...
				if (target != null) {
					...
					if (args.length == 1) {
						...
						if (args[0].equalsIgnoreCase("all")) { //$NON-NLS-1$
							...
							whiteName = Messages.getString("CE_Protect.0"); //$NON-NLS-1$
						}
						plugin.playerSettings.whitelistAdd(player, target.getType()
								.name(), args[0]);
						...
						if (args[1].equalsIgnoreCase("all")) { //$NON-NLS-1$
							...
							whiteName = Messages.getString("CE_Protect.1"); //$NON-NLS-1$
						}
						plugin.playerSettings.whitelistAdd(player, args[0].toUpperCase(), args[1]);
						sendSuccessMessage(player, blockName, whiteName);
						return true;
					} else {
		...
		private void sendSuccessMessage(Player player, String blockName,
			...
			plugin.say(
					player,
					ChatColor.GREEN,
					Messages.getString("CE_Whitelist.2") + blockName.toLowerCase() //$NON-NLS-1$
							+ Messages.getString("CE_Whitelist.1") + whiteName + Messages.getString("CE_Whitelist.0")); //$NON-NLS-1$ //$NON-NLS-2$
		}
