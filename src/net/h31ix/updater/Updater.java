...
public class Updater
{
     ...
     */
    public Updater(BlockOwn plugin, String slug, File file, UpdateType type, boolean announce)
    {
        this.plugin = plugin;
        this.type = type;
    ...
    private boolean versionCheck(String title)
    {
        if(type != UpdateType.NO_VERSION_CHECK)
        {
            String version = plugin.getDescription().getVersion();
                String remoteVersion = title.replace("v", ""); // Get the newest file's version number
                if(version.length()==2){
                	version+="0";
                }
